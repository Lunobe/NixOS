...
needs your Btrfs partition's UUID — this script will not guess it, since
getting this wrong touches disk layout. Below is your current block device
list; leave blank to skip this step entirely.
EOF
lsblk -f
echo
read -rp "Btrfs partition UUID: " BTRFS_UUID
if [ -n "$BTRFS_UUID" ]; then
  sudo mkdir -p /mnt/btrfs-root
  sudo mount -o subvolid=5 "/dev/disk/by-uuid/$BTRFS_UUID" /mnt/btrfs-root
...
sudo nixos-rebuild switch --flake "$TARGET#nixos" --extra-experimental-features "nix-command flakes"
